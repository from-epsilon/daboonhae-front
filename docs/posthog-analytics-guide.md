...
| `is_internal` | boolean | `true` | 내부 테스트 기기 여부 |
...
4. 이후 해당 브라우저의 모든 이벤트에 `is_internal=true` 부여
...
상단에 등록 해제 안내가 표시되고 이후 이벤트에는 `is_internal=false`가 붙는다.
...
4. Event properties에서 `is_internal = true`와 `app_environment = production`을 확인한다.
...
모든 KPI에는 기본적으로 `is_internal is not true` 필터를 적용한다.
...
- [ ] 등록 성공 안내 확인
- [ ] 이벤트에서 `is_internal=true` 확인
- [ ] 운영 대시보드에 `is_internal is not true` 필터 적용
- [ ] Internal QA 대시보드에 `is_internal is true` 필터 적용
