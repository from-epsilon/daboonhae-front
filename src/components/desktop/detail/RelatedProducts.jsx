...
export function RelatedProducts({ currentProduct, allProducts = [], onNavigate, limit = 3 }) {
  const categoryMatches = currentProduct?.categoryCode
    ? allProducts.filter(
        (p) => p.id !== currentProduct.id && p.categoryCode === currentProduct.categoryCode,
      )
    : [];
  const familyId = currentProduct?.family?.id ?? currentProduct?.familyId;
  const familyMatches = familyId
    ? categoryMatches.filter((p) => (p.family?.id ?? p.familyId) === familyId)
    : [];
  const related = (familyMatches.length > 0 ? familyMatches : categoryMatches)
  ...
  return (
    <section className="d-detail-section-block d-detail-related" aria-label="관련 제품">
      <div className="d-detail-related-header">
        <h2 className="d-detail-related-title">비슷한 제품</h2>
        <Link
