...
const DESKTOP_PROTEIN_SORT_OPTIONS = [
  ...
  ...PROTEIN_SORT_BASES.flatMap((base) =>
    PROTEIN_SORT_MODES.map((mode) => ({
      key: makeProteinSortKey(base.key, mode.key),
      label: `${base.label}순${PROTEIN_MODE_SUFFIX[mode.key] ?? ''}`,
      short: `${base.label}${PROTEIN_MODE_SUFFIX[mode.key] ?? ''}`,
    }))),
];

const RECOMMEND_SORT_HELP = '단백질 함량, 칼로리 대비 효율, 가격 대비 단백질, EAA·류신·BCAA 정보, 당류·지방·나트륨을 함께 보고 계산한 추천 점수 기반으로 정렬합니다. 아미노산 정보가 부족한 제품은 원재료의 단백질 품질을 참고해 보정합니다.';
